...
| e2e | test:e2e / e2e / playwright / cypress；`playwright.config.*` / `cypress.config.*` 是否存在 | 标注"无 e2e 框架"，⑦ 阶段将退化为浏览器手工验证或 unverified |
| dev server | dev / serve / start | 标注"无法本地启动"，并记录原因 |
| build | build | 标注构建耗时量级 |

**标注**: 每个命令的耗时量级（快<30s / 中30-120s / 慢>120s）；e2e 额外记录用例目录、命名约定、本地访问地址（baseUrl）。
...
生成 `.claude/fe-profile.md`，格式见 `../templates/项目画像模板.md`
